...
SYSCALL_DEFINE1(cs1550_down, struct cs1550_sem *, sem)
{
	mutex_lock(sem->lock);
	sem->value -= 1;
	//if no resources are available, add to sleeping queue and sleep
	if (sem->value < 0){
		//make a new node for this process
		struct Node* node = (struct Node*)kmalloc(sizeof(struct Node), GFP_KERNEL);
		node->sleeping_task = current;
		node->next = NULL;
		//if tail is null, then this is the only node in the queue
		if (sem->tail == NULL){
			sem->head = node;
			sem->tail = node;
		} else {
...
SYSCALL_DEFINE1(cs1550_up, struct cs1550_sem *, sem)
{
	...
	mutex_lock(sem->lock);
	sem->value += 1;
	//if there is a sleeping process, wake it up
	if (sem->value <= 0){
		//retrieve process from queue
		if (sem->head != NULL){
			struct Node* temp = sem->head;
			sleeping_task = sem->head->sleeping_task;
			//update queue
			sem->head = sem->head->next;
			//if head is null, there is no element
